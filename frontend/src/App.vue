      ...
      <main class="surface relative flex min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border divider lg:rounded-3xl">
        ...
        <template v-else>
          ...
          <div class="flex min-h-16 shrink-0 items-center gap-1 border-b divider px-3 sm:px-5">
            ...
            <template v-if="selected.length||activeThread"><span class="mx-1 h-5 border-l divider"></span><button class="icon-btn" :disabled="actionBusy" aria-label="归档" @click="modify([],['INBOX'])"><Icon name="archive" :size="18"/></button><button class="icon-btn" :disabled="actionBusy" aria-label="标记垃圾邮件" @click="modify(['SPAM'],['INBOX'])"><Icon name="alert" :size="18"/></button><button class="icon-btn" :disabled="actionBusy" :aria-label="folder==='TRASH'?'恢复邮件':'移入回收站'" @click="modify([],[],folder==='TRASH'?'untrash':'trash')"><Icon :name="folder==='TRASH'?'inbox':'trash'" :size="18"/></button><button class="icon-btn" :disabled="actionBusy" aria-label="标记为未读" @click="modify(['UNREAD'])"><Icon name="mail" :size="18"/></button><button class="icon-btn" :disabled="actionBusy" aria-label="应用标签" @click="labelChoice=customLabels[0]?.id||'';modal='apply-label'"><Icon name="tag" :size="18"/></button></template>
