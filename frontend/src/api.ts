...
export async function api<T = any>(path: string, method = 'GET', data?: unknown): Promise<T> {
  ...
  const response = await fetch('/api/v1' + path, { method, credentials: 'same-origin', headers: { ...(data && !form ? {'Content-Type': 'application/json'} : {}), ...(method !== 'GET' ? {'X-CSRF-Token': csrf} : {}) }, body: data === undefined ? undefined : form ? data : JSON.stringify(data) })
