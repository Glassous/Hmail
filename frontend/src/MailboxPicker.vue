  ...
  <div ref="root" class="mailbox-picker" @keydown="keydown" @focusout="focusout">
    <div ref="card" class="mailbox-card surface" :class="{ 'is-open': opened }" data-flip-id="mailbox-card">
      ...
      <div id="mailbox-options" class="mailbox-options" :role="opened ? 'listbox' : undefined" aria-label="选择邮箱">
        <button v-for="account in accounts" v-show="opened || account.id === modelValue" :key="account.id" type="button" :data-flip-id="'mailbox-' + account.id" :data-account="account.id" class="mailbox-option" :class="{ 'is-selected': opened && account.id === modelValue }" :role="opened ? 'option' : undefined" :aria-selected="opened ? account.id === modelValue : undefined" :tabindex="opened ? (highlighted === account.id ? 0 : -1) : 0" :title="account.email" :aria-label="opened ? account.email : '切换邮箱：' + account.email" @click="opened ? change(false, account.id, true) : change(true)">
